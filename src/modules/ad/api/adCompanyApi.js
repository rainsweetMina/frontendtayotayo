...
export async function fetchAdCompanies() {
    const res = await axios.get('/api/ad-company')
    return res.data
}

// 광고회사 등록
export async function createAdCompany(data) {
    const res = await axios.post('/api/ad-company', data)
    return res.data
}

// 광고회사 수정
export async function updateAdCompany(id, data) {
    const res = await axios.put(`/api/ad-company/${id}`, data)
    return res.data
}

// 광고회사 삭제
export async function deleteAdCompany(id) {
    return axios.delete(`/api/ad-company/${id}`)
}
